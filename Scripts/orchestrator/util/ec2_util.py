...
euwest_region = boto.ec2.RegionInfo(None, "eu-west-2", "ec2.eu-west-2.amazonaws.com")
useast_region = boto.ec2.RegionInfo(None, "us-east-2", "ec2.us-east-2.amazonaws.com")
        ...
        while len(status) < nbInstances:
            try:
                status = conn.get_all_instance_status(
                    instance_ids=getEc2InstancesId(
                        conn, "Name", {"tag:Name": tag}, True
                    )
                )
                print(status)
...
def getSpotReservation(
    ...
    spot = conn.request_spot_instances(
        price,
        image_id=ami_id,
        key_name=key,
        instance_type=instance,
        security_groups=security,
        block_device_map=disk,
        ebs_optimized=ebs_optimized,
        placement=placement,
    )  # Set to true again
    fulfilled = None
    while not fulfilled:
        fulfilled = conn.get_all_spot_instance_requests(
            [spot[0].id], filters={"status-code": "fulfilled"}
        )
        ...
        try:
            ...
            if disk_size:
                ...
                if not spot:
                    ...
                    reservation = getSpotReservation(
                        conn,
                        price,
                        ami_id,
                        key,
                ...
                if not spot:
                    ...
                    reservation = getSpotReservation(
                        conn, price, ami_id, key, instance, security, placement
                    )
            if name:
...
def stopEc2Instances(conn, instance_id_list):
    if len(instance_id_list):
        conn.stopEc2Instances(instance_id_list)
...
# Returns list of dns
def getEc2InstancesDns(conn, name_tag, filters=None, active=False):
    ...
    for reservation in reservations:
        for instance in reservation.instances:
            ...
            if ip_address and (
                not active
                or (instance.state == "pending" or instance.state == "running")
            ):
...
def getEc2InstancesPublicIp(conn, name_tag, filters=None, active=False):
    instances = []  # (instance_name, ip_address)
    if filters:
        reservations = conn.get_all_instances(filters=filters)
    else:
...
def startConnection(region, aws_key, aws_secret):
    ...
    if region == "eu-west-2" or region == "ca-central-1" or region == "us-east-2":
        ...
        reg = boto.ec2.get_region(
            region, aws_access_key_id=aws_key, aws_secret_access_key=aws_secret
        )
    try:
        conn = boto.ec2.connection.EC2Connection(aws_key, aws_secret, region=reg)
        return conn
    except:
...
def printSshConfig(instances):
    ...
    for instance_name, ip_address in instances:
        ...
        instance_name = (
            '"{0}"'.format(instance_name) if " " in instance_name else instance_name
        )
